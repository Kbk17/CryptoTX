...
const navigation = [
  { name: 'Add new transaction', href: '/add-transaction' },
  { name: 'My Transactions', href: '/transactions' },
];

const NavLogo = () => <img className='h-12 w-44' src={logo} alt='BorCash Logo' />;
      ...
      <nav className='flex items-center justify-between p-6 lg:px-8' aria-label='Global'>
        <div className='flex lg:flex-1'>
          <a href='/' className='-m-1.5 p-1.5'>
            <img className='h-12 w-38' src={logo} alt='BorCash Logo' />
          </a>
        </div>
      ...
      <Dialog as='div' className='lg:hidden' open={mobileMenuOpen} onClose={setMobileMenuOpen}>
        ...
        <Dialog.Panel className='fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-white dark:text-white dark:bg-boxdark px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10'>
          <div className='flex items-center justify-between'>
            <a href='/' className='-m-1.5 p-1.5'>
              <span className='sr-only'>BORCASH</span>
              <NavLogo />
            </a>
            <button
              type='button'
