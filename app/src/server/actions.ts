import { type User, type Task, type File, type BankDetails, type FiatCurrency, type Transaction } from 'wasp/entities';
...
export const createTransaction = async ({ data }, context) => {
  ...
  // Prosty sposób, aby upewnić się, że zarówno fiatCurrencyId, jak i bankDetailsId są podane
  ...
  const createdTransaction = await context.entities.Transaction.create({
    data: {
      ...
      lastModifiedByUserId: context.user.id,  // Ustawienie ID użytkownika tworzącego jako ostatniego modyfikatora
